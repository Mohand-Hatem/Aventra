...
export function CandidateSummaryPanel({ candidate }: SummaryPanelProps) {
  ...
  const strengths =
    (cvData?.aiAnalysis?.strengths?.length
      ? cvData.aiAnalysis.strengths
      : candidate.strengths) ?? [];
  const weaknesses =
    (cvData?.aiAnalysis?.weaknesses?.length
    ...
    return (
      <div className="w-full flex flex-col gap-4 p-4 rounded-xl border border-rose-200 bg-rose-50 dark:border-rose-800 dark:bg-rose-950/20">
        <p className="text-sm font-medium text-rose-600">
          Failed to load CV details. The CV may not be accessible.
        </p>
        {candidate.resumeUrl && (
          <a
    ...
    <div className="w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center px-4 py-2 bg-primary hover:bg-primary/95 text-white dark:bg-sky/40 dark:text-zinc-200 dark:hover:bg-sky/60 rounded-lg text-xs font-bold transition-colors shadow-sm"
          >
            Reach candidate
          </a>
        </div>
      ...
      <div className="flex flex-col gap-2">
        <p className="text-sm text-foreground leading-relaxed">
          <span className="text-primary dark:text-sky font-semibold mr-1">
            Strengths:
          </span>
        ...
        <p className="text-sm text-foreground leading-relaxed">
          <span className="text-sky dark:text-sky/80 font-semibold mr-1">
            Growth areas:
          </span>
          <span className="text-muted-foreground">
            {weaknesses.length > 0
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    ...
                    {contact.linkedin && (
                      ...
                      >
                        <IconBrandLinkedin
                          size={14}
                          className="text-primary dark:text-sky shrink-0"
                        />
                        <span className="truncate font-medium text-primary dark:text-sky hover:underline">
                          LinkedIn Profile
                        </span>
                      </a>
                    )}

...
export default function ResultsTable({
  ...
  const allEducation = useMemo(() => Array.from(new Set(candidates.flatMap(c => c.education?.map(e => e.degree || e.institution) || []).filter(Boolean))), [candidates]);
  const allStrengths = useMemo(() => Array.from(new Set(candidates.flatMap(c => c.strengths?.map(s => s.title) || []).filter(Boolean))), [candidates]);
  const allWeaknesses = useMemo(() => Array.from(new Set(candidates.flatMap(c => c.weaknesses?.map(w => w.title) || []).filter(Boolean))), [candidates]);
  ...
  const filteredCandidates = useMemo(() => {
    return candidates.filter((c) => {
      ...
      if (selectedEducations.length > 0) {
        const hasEdu = c.education?.some(e => selectedEducations.includes(e.degree || '') || selectedEducations.includes(e.institution || ''));
        if (!hasEdu) return false;
      }
      if (selectedStrengths.length > 0) {
        const hasStrength = c.strengths?.some(s => selectedStrengths.includes(s.title));
        if (!hasStrength) return false;
      }
      if (selectedWeaknesses.length > 0) {
        const hasWeakness = c.weaknesses?.some(w => selectedWeaknesses.includes(w.title));
        if (!hasWeakness) return false;
      }
      
      return true;
    });
  }, [candidates, minAtsScore, minMatchScore, selectedEducations, selectedStrengths, selectedWeaknesses]);
  ...
  function toggleFilter(list: string[], setList: (l: string[]) => void, val: string) {
          ...
          <Dialog>
            <DialogTrigger asChild>
              <button className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-muted relative">
                ...
                {(selectedEducations.length > 0 || selectedStrengths.length > 0 || selectedWeaknesses.length > 0 || minAtsScore > 0 || minMatchScore > 0) && (
              ...
              <div className="flex flex-col gap-6 py-4">
                <div className="space-y-3">
                  <div className="flex justify-between text-sm font-semibold text-foreground">
                    ...
                    <span className="text-primary tabular-nums">{minAtsScore}%</span>
                  </div>
                  <input
                    type="range" min="0" max="100" value={minAtsScore}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm font-semibold text-foreground">
                    ...
                    <span className="text-primary tabular-nums">{minMatchScore}%</span>
                  </div>
                  <input
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allEducation.map(edu => {
                        const active = selectedEducations.includes(edu as string);
                        return (
                          <button key={edu as string} onClick={() => toggleFilter(selectedEducations, setSelectedEducations, edu as string)}
                            className={cn("rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors border", active ? "bg-primary text-primary-foreground border-primary" : "bg-muted text-muted-foreground border-transparent hover:border-border")}
                          >
                            {edu as string}
                          </button>
                        );
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allStrengths.map(strength => {
                        const active = selectedStrengths.includes(strength as string);
                        return (
                          <button key={strength as string} onClick={() => toggleFilter(selectedStrengths, setSelectedStrengths, strength as string)}
                            className={cn("rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors border", active ? "bg-emerald-500 text-white border-emerald-500" : "bg-muted text-muted-foreground border-transparent hover:border-border")}
                          >
                            {strength as string}
                          </button>
                        );
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allWeaknesses.map(weakness => {
                        const active = selectedWeaknesses.includes(weakness as string);
                        return (
                          <button key={weakness as string} onClick={() => toggleFilter(selectedWeaknesses, setSelectedWeaknesses, weakness as string)}
                            className={cn("rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors border", active ? "bg-rose-500 text-white border-rose-500" : "bg-muted text-muted-foreground border-transparent hover:border-border")}
                          >
                            {weakness as string}
                          </button>
                        );
                ...
                <div className="pt-2">
                  <button 
                    onClick={() => { setMinAtsScore(0); setMinMatchScore(0); setSelectedEducations([]); setSelectedStrengths([]); setSelectedWeaknesses([]); }}
